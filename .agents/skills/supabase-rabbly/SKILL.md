...
## Database Schema (Current — synced 2026-09-10)
...
### `sessions` table
Represents a learning session (solo or classroom).
...
CREATE TABLE public.sessions (
  ...
  status       text,        -- e.g. 'active', 'ended'
  board_state  jsonb,       -- whiteboard snapshot
  created_at   timestamptz DEFAULT now(),
  updated_at   timestamptz DEFAULT now()
);
```
...
### `sprints` table
Learning sprints / study plans created by a user.
...
CREATE TABLE public.sprints (
  ...
  resources        jsonb,   -- array of resource links/refs
  created_at       timestamptz DEFAULT now(),
  updated_at       timestamptz DEFAULT now()
);
```
